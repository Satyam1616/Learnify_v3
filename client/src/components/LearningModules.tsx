          ...
          {modules.map((module, index) => (
            ...
            >
              <div className="mb-4 sm:mb-6 text-center">
                <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-2 group-hover:text-[hsl(25,95%,53%)] transition-colors leading-tight">
                  {module.title}
                </h3>
                <div className="flex items-center justify-center space-x-2">
                  <div className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full ${module.available ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                  <p className={`font-semibold text-base sm:text-lg ${module.available ? 'text-[hsl(25,95%,53%)]' : 'text-gray-500'}`}>
                    {module.subtitle}
                  </p>
                </div>
              </div>
              ...
              <div className="text-center">
                <div className={`inline-flex items-center px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-colors ${module.available
                  ? 'bg-[hsl(25,95%,53%)] text-white group-hover:bg-[hsl(35,95%,60%)]'
