  ...
  return (
    <section className="py-20 bg-[hsl(220,13%,97%)] relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-20 left-20 w-40 h-40 border-4 border-[hsl(219,79%,37%)] rounded-full"></div>
        <div className="absolute bottom-20 right-20 w-32 h-32 bg-[hsl(219,79%,37%)] rounded-full opacity-10"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-8 lg:gap-12">
          {modules.map((module, index) => (
            <div 
              key={index} 
              className={`bg-white rounded-3xl p-8 shadow-lg hover:shadow-2xl transition-all duration-500 group transform hover:-translate-y-4 animate-scale-in ${
                !module.available ? 'grayscale hover:grayscale-0' : ''
              }`}
